...
import org.elasticsearch.index.codec.vectors.diskbbq.calibrate.CalibrationQueries;
...
public class IvfAutoCalibration {
    ...
    public IvfSegmentConfig resolve(FieldInfo fieldInfo, MergeState mergeState, IvfSegmentConfig codecDefault) throws IOException {
        ...
        FloatVectorValues floatVectorValues = CalibrationUtils.build(fieldInfo, mergeState);
        ...
        if (numVectors < MIN_VECTORS_FOR_CALIBRATION) {
            return new IvfSegmentConfig(codecDefault.quantEncoding(), codecDefault.usePrecondition(), DEFAULT_CALIBRATED_OVERSAMPLE);
        }
        ...
        try {
            ...
            return new IvfSegmentConfig(codecDefault.quantEncoding(), false, DEFAULT_CALIBRATED_OVERSAMPLE);
        }
    }
    ...
    /**
     * Runs full calibration on {@code floatVectorValues}. Merge uses {@link CalibrationUtils#build(FieldInfo, MergeState)}
     * for a reader-backed random-access view over input segments.
     */
